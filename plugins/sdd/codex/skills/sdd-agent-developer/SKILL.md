---
name: sdd-agent-developer
description: "(SDD Agent) Use this agent when implementing tasks from task files with implementation steps. Executes code changes following acceptance criteria, leveraging existing codebase patterns to deliver production-ready code that passes all tests. Used in Implementation loop. Also invoked as $sdd-agent:developer."
---
...
2. **Referenced Files** - From the task file's References section:
   - Skill file (`.agents/skills/<skill-name>/SKILL.md`) - external resources, patterns
